...
- Le pilote ODBC HFSQL (PCSoft) est **propriétaire** : son pack (`*.zip` Linux) ne doit **jamais** être committé dans le repo public (`.gitignore` : `hfsql-odbc/`). L'utilisateur le dépose dans `./hfsql-odbc/` ; `docker-entrypoint.sh` l'installe **automatiquement** au démarrage du conteneur : dézippage dans `/opt/hfsql-odbc/lib`, détection du `*hfo64.so` (nom variable selon version du pack), écriture **directe** de la section `[HFSQL]` dans `/etc/odbcinst.ini` (lisible par unixODBC ET iODBC), export `LD_LIBRARY_PATH` (libs `wd*.so` WinDev).
  - Si unixODBC remonte `01000 Can't open lib 'HFSQL' : file not found` → la section `[HFSQL]` est absente de `/etc/odbcinst.ini` que lit unixODBC : vérifier le pack dans `./hfsql-odbc/` et les logs `[entrypoint]` (`docker compose logs smart | grep entrypoint`).
  ...
  - **Pont iODBC `hfcli` (Linux)** : le driver HFSQL ne fonctionne qu'avec **iODBC** (doc PCSoft + communauté : erreur `0 U` / `SQLDriverConnect` corrompue dès qu'on passe par unixODBC, ex. pyodbc/node-odbc). node-odbc est unixODBC uniquement → l'app passe sous Linux par `backend/hfcli/hfcli.c` (binaire `/usr/local/bin/hfcli`, compilé dans un stage Docker gcc + `libiodbc2-dev`, COPY vers runtime ; pas de conflit apt car ce stage ne contient pas unixodbc-dev). Ops JSON sur stdout : `test`, `tables`, `columns TABLE`, `query "SQL" [PARAMS...]` ; échec ODBC → `{"ok":false,"message":"STATE message"}` + exit 2. Params numériques liées en SQL_INTEGER (LIMIT/OFFSET), sinon en SQL_VARCHAR ; `\u0003NULL` (`\x03NULL`) = param NULL. iODBC : `SQLColumns` prend 8 args (pas de `SQL_NULL_HWND`), headers dans `/usr/include/iodbc`. Fallback node-odbc conservé (dev Windows).
  - **Quirk iODBC + HFSQL** : `SQLColumns` renvoie `COLUMN_NAME` **vide** (REMARKS/size/ordinal OK). Les vrais noms viennent des métadonnées de `SELECT * FROM "t" WHERE 1=0` (`SQLDescribeCol`) — `hfcli columns` fusionne les deux (noms par position = ordinal). `query` utilise aussi `SQLDescribeCol` pour ses clés (même API que `columns` pour que `row[col.name]` corresponde). Longs noms avec accents (ex. `Coordonnéesgps`) peuvent ressortir tronqués (cosmétique).
  - **node-odbc (Windows)** : `tables`/`columns` exigent des **arguments positionnels** (catalog, schema, table, type) — pas d'objet options (`first argument must be a string or null`).
  - Windows : driver HFSQL v25 déjà enregistré (`C:\Program Files (x86)\Common Files\PC SOFT\25.0\ODBC\Win64x86\wd250hfo64.dll`) → permet de tester l'app en dev local contre la vraie base (`Driver={HFSQL};Server Name=...;Server Port=14900;DATABASE=SIT;UID=admin;PWD=...`).
...
- **Piège compose** : `LOCAL_DB_PATH` (et tout chemin de volume) doit être le chemin **dans le conteneur** (`/app/backend/data/smart-local.db`), PAS le chemin hôte NAS (`/volume1/docker/smart/data/...`) — sinon SQLite écrit dans la couche éphémère du conteneur et tout est perdu à la recréation. L'UI (Paramètres → « Stockage des paramètres ») et le log `[db-local]` au démarrage montrent le chemin réellement utilisé.
